...
class Grafo:
    def __init__(self, dirigido: bool = False) -> None:
        ...
        self.__flowMatriz:list[list[float]] = list()
    ...
    def getCapacidade(self, u: int, v: int) -> float:
        return self.adjacencyMatriz[u][v] + self.flowMatriz[u][v]
    ...
    def ler(self, filePath: str) -> None:
        ...
        self.__flowMatriz = [[0 for _ in range(self.__qtdVertex)] for _ in range(self.__qtdVertex)]
    ...
    def getcopy(self):
        cp = Grafo()
        cp.adjacencyMatriz = deepcopy(self.adjacencyMatriz)
        cp.vertexes = deepcopy(self.vertexes)
        cp.flowMatriz = deepcopy(self.flowMatriz)
        cp.qtdVertex = self.qtdVertex
        cp.qtdEdge = self.qtdEdge
        return cp
    ...
    @property
    def flowMatriz(self) -> list[list[float]]:
        return self.__flowMatriz

    @flowMatriz.setter
    def flowMatriz(self, v: list[list[float]]) -> None:
        self.__flowMatriz = v
