...
%    filme(id, titulo, ano, diretor, nacionalidade).
a :- findall(T, (filme(_, T, Ano, _, _), Ano < 2001), L),
     writeln(L),
     halt.

b :- findall(T, (filme(_, T, Ano, _, _), 2001 =< Ano, Ano  =< 2004), L),
     writeln(L),
     halt.

c :- findall(Titulo, filme(_, Titulo, _, _, br), L),
     sort(L, L1),
     writeln(L1),
     halt.

d :- findall(Titulo, (filme(_, Titulo, _, _, P), P \== br), L),
     sort(L, L1),
     writeln(L1),
     halt.
