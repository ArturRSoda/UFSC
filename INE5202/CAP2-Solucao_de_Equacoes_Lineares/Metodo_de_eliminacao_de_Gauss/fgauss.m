...
%Analise de tipos de sistemas: Determinado, Indeerminado e Impossivel
	...
	if (A(n,n) != 0) %Se A(n,n) for residuo pequeno? abs(A(n,n)) > 1e**(-14)
		X(n, 1) = A(n,n+1)/A(n,n);

	else
		if (A(n,(n+1)) == 0) %Se A(n,(n+1)) for residuo pequeno? abs(A(n,(n+1))) > 1e**(-14)
			disp("Sistema Indeterminado")
			X(n,1) = 7; % Chute controlado	
		else
			disp("Sistema Impossivel, arrume seu sistema!!")
			X(n) = NaN;
			exit;
		end
	end
